...
//        });
     ...
     */
    @Override
    public List<Permission> selectAllMenuGuli() {
        //查询菜单表所有数据
        QueryWrapper<Permission> wrapper = new QueryWrapper<>();
        wrapper.orderByDesc("id");
        List<Permission> permissionList = baseMapper.selectList(wrapper);
        ...
        List<Permission> permissions = bulidPermissionNew(permissionList);

        return permissions;
    }

    private List<Permission> bulidPermissionNew(List<Permission> permissionList) {

        //用于数据最终封装
        List<Permission> finalNode = new ArrayList<>();

        //把所有菜单list集合遍历，得到顶层菜单pid=0菜单，设置level是1
        for (Permission permission : permissionList) {
            ...
            if ("0".equals(permission.getPid())){
                ...
                //根据顶层菜单，向里面进行查询子菜单，封装到finalNode里面
                finalNode.add(selectChildrenNew(permission,permissionList));
            }
        }
        return finalNode;
    }

    private Permission selectChildrenNew(Permission permission, List<Permission> permissionList) {
        //因为向一层菜单里面放二层菜单，二层里面还要放三层。。。 把对象初始化
        permission.setChildren(new ArrayList<>());
         //遍历所有菜单list集合，进行判断比较，比较id和pid值是否相等
        for (Permission it : permissionList) {
            if (permission.getId().equals(it.getPid())) {
                ...
                //把查询出来的子菜单放到父菜单里面
                permission.getChildren().add(selectChildrenNew(it,permissionList));
            }
        }
        return permission;
    }
